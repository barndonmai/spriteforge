...
class GeminiImageProviderTests(unittest.TestCase):
    ...
    def test_object_generation_prompt_uses_generic_template_with_summary_fields(self) -> None:
        ...
        self.assertIn("use subtle dimensional form when it improves readability at sprite size", prompt)
        self.assertIn("prefer a gentle 3/4 sprite view with a visible side plane or top rim", prompt)
        self.assertIn("use simple pixel-art shading to describe volume and edge planes", prompt)
